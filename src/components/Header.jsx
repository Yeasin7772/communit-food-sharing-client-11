
const Header = () => {
    return (

        <div>
            <div className="hero min-h-screen ">
                {/* bg-[#F9F6E2] */}
                <div className="hero-content flex-col lg:flex-row-reverse px-5">
                    <img data-aos="fade-down"
                        data-aos-easing="linear"
                        data-aos-duration="1500" src="https://i.ibb.co/B6Y5MfL/21683580-Tiny-people-standing-near-box-of-donation-food-for-delivery.jpg" className=" max-w-full sm:h-52  md:h-96 rounded" />
                    <div data-aos="fade-up"
                        data-aos-duration="3000" className="">
                        <h1 className="text-3xl font-bold text-blue-500">JOIN WITH US YOUR ATTENTION IS CHANGED THE PART OF WORLD.</h1>
                        <p className="py-6"><small>Cupiditate qui molestias fugit voluptatibus laudantium maxime <br />voluptate corrupti ab repudiandae dolor repellendus? laudantium <br /> maxime  voluptate corrupti ab repudiandae dolor repellendus?</small></p>
                        <button className="btn btn-sm rounded-xl bg-orange-600 text-white">Learn More</button>
                    </div>
                </div>
            </div>

            {/* <div className="hero min-h-full lg:h-[80vh]" style={{ backgroundImage: 'url(http://unlockdesizn.com/html/non-profit/be-ahand/demo/images/about/1.jpg)' }}>
            <div className="hero-overlay bg-opacity-40"></div>
            <div className="hero-content text-center text-neutral-content">
                <div className=" px-12">
                    <h1 className="mb-5 text-3xl font-bold">JOIN WITH US, <br /> YOUR ATTENTION IS CHANGED THE PART OF WORLD.</h1>
                    <p className="mb-5"><small>Cupiditate qui molestias fugit voluptatibus laudantium maxime <br />voluptate corrupti ab repudiandae dolor repellendus? laudantium maxime voluptate corrupti ab repudiandae dolor repellendus?</small></p>
                    <div className="flex justify-center gap-6 text-white">
                        <button className="btn btn-outline btn-primary">Donation Now</button>

                        <button className="btn btn-outline btn-warning">Join With us</button>

                    </div>
                </div>

            </div>
        </div> */}
        </div>


    );
};

export default Header;

